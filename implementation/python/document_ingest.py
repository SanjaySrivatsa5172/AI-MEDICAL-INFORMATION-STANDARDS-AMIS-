...
class DocumentIngest:
    ...
    @staticmethod
    def from_text(text: str, filename: Optional[str] = None, source_kind: str = "paste") -> IngestedDocument:
    ...
    def from_bytes(data: bytes, filename: str) -> IngestedDocument:
        ...
        if suffix == ".pdf":
            text = DocumentIngest._pdf_to_text(data)
            kind = "pdf"
        elif suffix in {".txt", ".md", ".text", ""}:
            text = DocumentIngest._decode(data)
            kind = "txt"
        else:
            # Attempt text first; PDFs without a suffix still fail decode cleanly.
            if data[:5] == b"%PDF-":
                text = DocumentIngest._pdf_to_text(data)
                kind = "pdf"
            else:
        ...
        return DocumentIngest.from_text(text, filename=name, source_kind=kind)
